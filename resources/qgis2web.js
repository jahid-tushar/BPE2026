...
var map = new ol.Map({
    ...
    view: new ol.View({
         maxZoom: 28, minZoom: 1, projection: new ol.proj.Projection({
            code: 'EPSG:4326',
            //extent: [88.008169, 20.590741, 92.679603, 26.634545],
            units: 'degrees'})
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([84.006583, 19.998211, 97.975119, 27.690498], map.getSize());
...
var Title = new ol.control.Control({
    element: (() => {
        ...
        titleElement.innerHTML = '<h2 class="project-title">Bangladesh Parliamentary Election 2026: Winning Party by Upazila</h2>';
        return titleElement;
    })(),
    ...
    element: (() => {
        ...
        if (2514 > 240) {
            ...
            window.showAbstract = function() {
                ...
                linkElement.innerHTML = 'This interactive web map displays an approximate visualization of the winning political parties for each Upazila in Bangladesh based on the results of the 13th Jatiya Sangsad (Parliamentary) Election held on 12 February 2026.<br /><br />The map aggregates national constituency-level results (300 parliamentary seats) to the Upazila level (approximately 495 sub-districts) by assigning each Upazila to its primary or corresponding constituency. In cases where constituencies span multiple Upazilas or are partially split, the assignment uses the dominant or most commonly reported winning party.<br /><br />Key national highlights (as of mid-February 2026, for ~297–299 declared seats):<br />- Bangladesh Nationalist Party (BNP) secured a landslide victory with approximately 208–213 seats.<br />- Bangladesh Jamaat-e-Islami (and allies) won around 68–77 seats, marking their strongest performance historically.<br />- Other parties including National Citizen Party (NCP ~6 seats), Independents (~7–8 seats), Islami Andolan Bangladesh, Bangladesh Khelafat Majlis, and others claimed the remainder.<br /><br />**Important disclaimer on accuracy**: This map is for informational, educational, and visualization purposes only. The data is compiled from publicly available media reports and summaries as of February 14, 2026, and may not reflect final official gazette notifications, any pending recounts, postponed constituencies (e.g., certain areas in Sherpur or Chattogram), or minor boundary adjustments. Upazila-to-constituency mappings are based on standard administrative delimitations and are approximate; they do not account for rare split constituencies in full detail. Users should cross-verify with official sources from the Bangladesh Election Commission (EC) for the most accurate and up-to-date information.<br /><br />Primary data sources:<br />- Election results and winning parties: Bangladesh Election Commission (EC) unofficial/official tallies as reported by Dhaka Tribune (full division-wise winners list), The Daily Star, Prothom Alo, Al Jazeera, BBC, CNN, Wikipedia (2026 election summary), and other reputable outlets.<br />- Administrative boundaries, Upazila names, and approximate centroids (lat/long): Derived from open geospatial datasets including GADM, OpenStreetMap, Bangladesh Bureau of Statistics (BBS), and public GeoJSON repositories (e.g., bangladesh-geojson on GitHub, Wikidata, or similar sources used in your bgd_admin3.shp layer).<br /><br />Last compiled/updated: February 14, 2026. Always refer to the official EC gazette for authoritative results.';
            }
